...
class ModeloTestimonios{
	...
	static public function mdlMostrarTestimonios($tabla, $item, $valor){
		...
		if($item != null && $valor != null){
			...
			$stmt = Conexion::conectar()->prepare("SELECT *FROM $tabla ORDER BY id_test DESC");
	...
	static public function mdlMostrarTestimoniosInnerJoin($tabla1, $tabla2, $tabla3, $item, $valor){
			...
			$stmt = Conexion::conectar()->prepare("SELECT $tabla1.*, $tabla2.*, $tabla3.* FROM $tabla1 INNER JOIN $tabla2 ON $tabla1.reserva_id = $tabla2.id_reserva INNER JOIN $tabla3 ON $tabla1.usuario_id = $tabla3.id_u WHERE $item = :$item");

...
class ControladorBanner{
	...
	=============================================*/
		...
		if(isset($_FILES["subirBanner"]["tmp_name"]) && !empty($_FILES["subirBanner"]["tmp_name"])){  /* es una cosa venga la variable y otra cosa vemga vacia  */

			list($ancho, $alto) = getimagesize($_FILES["subirBanner"]["tmp_name"]);

			$nuevoAncho = 1440;   /* si subista otra imagen con otras dimensiones ya php se encarga de redensionarla  */
			$nuevoAlto = 600;
	...
	static public function ctrEliminarBanner($id, $ruta){
		...
		unlink("../".$ruta);   /* borrar la imagen actual del servidor */

		$tabla = "banner";

		$respuesta = ModeloBanner::mdlEliminarBanner($tabla, $id);  /* borrar la ruta imagen desde la base de datos */

...
class ControladorCategorias{
			...
			if(preg_match('/^[a-zA-Z0-9-]+$/', $_POST["rutaCategoria"]) && 
			   ...
			   preg_match('/^[0-9.]+$/', $_POST["americano_baja"])){
				...
				if(isset($_FILES["subirImgCategoria"]["tmp_name"]) && !empty($_FILES["subirImgCategoria"]["tmp_name"])){
					...
					$directorio = "vistas/img/".strtolower($_POST["tipoCategoria"]);	/* directorio es el nombre de la categoria y convertido en miniscula asi tenemos deseñada la aerquitectura */
					...
					$datos = array("ruta" => $_POST["rutaCategoria"],
								   ...
								   "incluye" => $_POST["caracteristicasCategoria"],  /* acdena texto json  */
					...
					if($_FILES["editarImgCategoria"]["type"] == "image/jpeg"){
					    ...
					    echo'<script>
					        	...
					        	setTimeout(function(){
					        		...
					        		window.location="http://localhost/reservas-h/backend/categorias";   
	...
	static public function ctrEliminarCategoria($id, $ruta, $tipo){
		...
		unlink("../".$ruta);     /* => elemino la ruta del imagen */ /* es decir elimino la imagen */
		rmdir("../vistas/img/".strtolower($tipo));   /* elemino la carpeta de la imagen , sabemos que la carpeta de la imagen se crea en el nombre de la categoria en miniscula */

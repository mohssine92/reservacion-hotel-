			   ...
			   preg_match('/^[\/\=\\&\\;\\_\\*\\"\\<\\>\\?\\¿\\!\\¡\\:\\,\\.\\0-9a-zA-ZñÑáéíóúÁÉÍÓÚ ]+$/', $_POST["descripcionRecorrido"])){
				...
				if(isset($_FILES["subirImgPeqRecorrido"]["tmp_name"]) && !empty($_FILES["subirImgPeqRecorrido"]["tmp_name"])){
					...
					if($_FILES["subirImgPeqRecorrido"]["type"] == "image/jpeg"){
						...
						function generat($length = 10) { 
							return substr(str_shuffle("0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"), 0, $length); 
						} 
						
						$aleatorio =  generat($length = 10);
						...
						imagejpeg($destino, $rutaImgPeq);	
						 ...
						 function generar($length = 10) { 
							return substr(str_shuffle("0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"), 0, $length); 
						} 
						 
						 $aleatorio = generar($length = 10); 
				...
				if(isset($_FILES["subirImgGrandeRecorrido"]["tmp_name"]) && !empty($_FILES["subirImgGrandeRecorrido"]["tmp_name"])){
					...
					if($_FILES["subirImgGrandeRecorrido"]["type"] == "image/jpeg"){
						...
						function generacion($length = 10) { 
							return substr(str_shuffle("0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"), 0, $length); 
						} 
						
						$aleatorio =  generacion($length = 10);
						...
						$aleatorio = generation($length = 10); 
				...
				$datos = array("titulo" => strtoupper($_POST["tituloRecorrido"]),   /*  strtoupper() Devuelve el string con todos los caracteres alfabéticos convertidos a mayúsculas.  */
		...
		if(isset($_POST["idRecorrido"])){
			...
			if(preg_match('/^[a-zA-Z0-9ñÑáéíóúÁÉÍÓÚ ]+$/', $_POST["editarRecorrido"]) && 
			   preg_match('/^[\/\=\\&\\;\\_\\*\\"\\<\\>\\?\\¿\\!\\¡\\:\\,\\.\\0-9a-zA-ZñÑáéíóúÁÉÍÓÚ ]+$/', $_POST["editarDescripcionRecorrido"])){
				...
				if(isset($_FILES["editarImgPeqRecorridoActual"]["tmp_name"]) && !empty($_FILES["editarImgPeqRecorridoActual"]["tmp_name"])){	
				...
				if(isset($_FILES["editarImgGrandeRecorridoActual"]["tmp_name"]) && !empty($_FILES["editarImgGrandeRecorridoActual"]["tmp_name"])){				
